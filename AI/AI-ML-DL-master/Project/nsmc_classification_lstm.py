...
import numpy as np
from Project.lstm_model import lstm_model
...
fig_file_name = '4_lstm_pyplot.png'
...
def run_lstm(params) :
    ...
    dropout_keep_prob = float(params[3])
    num_epochs = int(params[4])
    batch_size = int(params[5])
    evaluate_every = int(params[6])
    learn_rate = float(params[7])
    num_layers = int(params[8])
    ...
    if not(os.path.isdir('./lstm_model')) :
        os.makedirs(os.path.join('./lstm_model'))
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            train_op = optimizer.minimize(lstm.cost, global_step=global_step)
            #grads_and_vars = optimizer.compute_gradients(lstm.cost)
            ...
            def train_step(x_batch, y_batch):
                ...
                _, step, cost, accuracy = sess.run([train_op, global_step, lstm.cost, lstm.accuracy], feed_dict)
            ...
            def dev_step(x_batch, y_batch, epoch):
                ...
                step, cost, accuracy, dev_pred = sess.run([global_step, lstm.cost, lstm.accuracy, lstm.predictions], feed_dict)
            ...
            for epoch in range(num_epochs):
                for len_batch in range(len(train_x_batches)):
                    ...
                    if current_step % evaluate_every == 0:
                        dev_step(x_dev, y_dev, epoch, 'dev')
...
if __name__ == "__main__":
    default_param = [sys.argv[0], 100, 30, 0.5, 20, 1000, 150, 0.001, 2]

    if len(sys.argv)==1:
        run_lstm(default_param)
    else :
        print(sys.argv)
        run_lstm(sys.argv)